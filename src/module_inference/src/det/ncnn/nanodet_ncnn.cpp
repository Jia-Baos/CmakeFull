...
NanoDetNCNN::NanoDetNCNN(const std::string &config_path)
{
    // 加载模型，配置模型和推理参数
    ...
    this->Net->opt.num_threads = m_thread_num;
    this->Net->load_param(ncnn_param_path.c_str()); // 此处加载顺序错误会 Segmentation fault (core dumped)
    this->Net->load_model(ncnn_bin_path.c_str());
    ...
    std::cout << "model weight: " << ncnn_param_path << std::endl;
...
std::shared_ptr<DetOutput> NanoDetNCNN::Detect(const cv::Mat &img)
{
    ...
    ResizeUniform(img, image, cv::Size(this->m_input_width, this->m_input_height), this->m_object_rect); // -> 416*416
    std::cout << "ResizeUniform(), src size: " << image.size() << std::endl;

    ncnn::Mat input_tensor = ncnn::Mat::from_pixels(image.data, ncnn::Mat::PIXEL_BGR, this->m_input_width, this->m_input_height);
    ...
    ex.set_num_threads(m_thread_num);
#if NCNN_VULKAN
    ...
    decode_infer(out, center_priors, m_confidence_threshold, results);
...
std::shared_ptr<DetModel> NanoDetNCNN::GetModel(const std::string &config_path)
{
    std::shared_ptr<DetModel> model(new NanoDetNCNN(config_path));
    if (!model->IsValid()) {
