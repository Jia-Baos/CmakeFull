...
NanoDetOPENVINO::NanoDetOPENVINO(const std::string &config_path)
{
    // 加载模型，配置模型和推理参数
    ...
    // 权重文件与配置文件同级
    std::string openvino_bin_path = replaceExtension(config_path, ".bin");
    std::string openvino_xml_path = replaceExtension(config_path, ".xml");

    ov::Core core{};
    std::shared_ptr<ov::Model> model = core.read_model(openvino_xml_path);
    // std::shared_ptr<ov::Model> model = core.read_model(openvino_xml_path, openvino_bin_path);

    network_ = core.compile_model(model, "CPU");
    infer_request_ = network_.create_infer_request();
    ...
    std::cout << "model weight: " << openvino_bin_path << std::endl;
...
std::shared_ptr<DetOutput> NanoDetOPENVINO::Detect(const cv::Mat &img)
{
    ...
    ov::Tensor input_blob = infer_request_.get_tensor(input_name);
    ...
    const ov::Tensor pred_blob = infer_request_.get_tensor(output_name);
    const float *pred = pred_blob.data<const float>();
    ...
    decode_infer(pred, center_priors, m_confidence_threshold, results);
...
std::shared_ptr<DetModel> NanoDetOPENVINO::GetModel(const std::string &config_path)
{
    std::shared_ptr<DetModel> model(new NanoDetOPENVINO(config_path));
    if (!model->IsValid()) {
...
void NanoDetOPENVINO::decode_infer(const float* pred, const std::vector<NanoDet::CenterPrior> &center_priors, const float threshold, std::vector<std::vector<NanoDet::BoxInfo>> &results)
{
    ...
    for (int idx = 0; idx < kNumPoints; idx++) {
        ...
        for (int label = 0; label < num_class; label++) {
            if (pred[idx* kNumChannels + label] > score) {
                score = pred[idx* kNumChannels + label];
                cur_label = label;
            }
        }
        if (score > threshold) {
            const float *bbox_pred = pred + idx * kNumChannels + num_class;
            results[cur_label].push_back(disPred2Bbox(bbox_pred, cur_label, score, kCtX, kCtY, kStride));
        }
    }
}

NanoDet::BoxInfo NanoDetOPENVINO::disPred2Bbox(const float *dfl_det, const int label, const float score, const int x, const int y, const int stride)
{
    float ct_x = x * stride;
    float ct_y = y * stride;
...
void NanoDetOPENVINO::nms(std::vector<NanoDet::BoxInfo> &input_boxes, const float nms_thresh)
{
